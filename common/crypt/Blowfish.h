...
{
	//Constructors
    SBlock(AUInt l=0, AUInt r=0) : m_uil(l), m_uir(r) {}
	//Copy Constructor
...
public:
    ...
    CBlowFish(AUChar* ucKey, size_t n, const SBlock& roChain = SBlock(0UL,0UL));

	//Resetting the chaining block
	void ResetChain() { m_oChain = m_oChain0; }

	// Encrypt/Decrypt Buffer in Place
    void Encrypt(AUChar* buf, size_t n, AInt iMode=ECB);
    void Decrypt(AUChar* buf, size_t n, AInt iMode=ECB);

	// Encrypt/Decrypt from Input Buffer to Output Buffer
    void Encrypt(const AUChar* in, AUChar* out, size_t n, AInt iMode=ECB);
    void Decrypt(const AUChar* in, AUChar* out, size_t n, AInt iMode=ECB);
...
private:
    ...
    AUInt m_auiP[18];
    AUInt m_auiS[4][256];
    static const AUInt scm_auiInitP[18];
    static const AUInt scm_auiInitS[4][256];
};

//Extract low order byte
inline AUChar Byte(AUInt ui)
{
    return (AUChar)(ui & 0xff);
}

//Function F
inline AUInt CBlowFish::F(AUInt ui)
{
